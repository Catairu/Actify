    ...
    def log_visualizations(self, x, x_hat, y, stage="train"):
        ...
        for col_idx, (act_id, batch_idx) in enumerate(targets.items()):
            ...
            for row_idx in range(3):
                ...
                if row_idx == 0:
                    ax.set_title(
                        f"Activity: {act_name}", fontsize=14, fontweight="bold"
                    )
                if col_idx == 0:
        ...
        self.logger.experiment.log(
            {f"comparison/{stage}_sit_vs_stand": wandb.Image(fig)}
        )
        plt.close(fig)
...
class LSTMClassifier(lit.LightningModule):
    def __init__(self, cfg, pretrained_encoder):
        ...
        ).device  ## To solve GPU/CPU mismatch errors
        ...
        self.lstm = instantiate(
            cfg.rnn_block, input_size=lstm_input_dim
        )  ## Passing the automatic input size
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch
        y_hat = self(x)
        w_list = self.cfg.get("lstm_weights", [1.0] * self.cfg.num_classes)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # To check for the type of error in our most critical classes: Sitting=3, Standing=4
        watch_classes = [3, 4]
        ...
        for i in range(len(y)):
            ...
            is_problem_class = (true_label in watch_classes) or (
                pred_label in watch_classes
            )
            ...
            if is_problem_class and is_wrong:
                ...
                print(
                    f"   True: {self.class_names[true_label]}  -->  Pred: {self.class_names[pred_label]}"
                )
                print(
                    f"   Confidence: {confidence:.4f} (Uncertainty: {1.0 - confidence:.4f})"
                )

                print("   --- Scores ---")
                print(
                    f"   Sitting (3):  Logit={logits[i][3]:.2f} | Prob={probs[i][3]:.4f}"
                )
                print(
                    f"   Standing (4): Logit={logits[i][4]:.2f} | Prob={probs[i][4]:.4f}"
                )
        ...
        self.precision.update(logits, y)
        self.recall.update(logits, y)
        self.f1_per_class.update(logits, y)
    ...
    def on_test_epoch_end(self):
        ...
        if self.logger:
            self.logger.experiment.log(
                {"confusion_matrix": wandb.Image(fig), "global_step": self.global_step}
            )
